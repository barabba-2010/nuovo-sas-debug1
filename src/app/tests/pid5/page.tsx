...
import PID5Test from '../../components/tests/pid5/PID5Test';
import { type PID5TestState } from '../../components/tests/pid5/PID5Test';

export default function PID5TestPage() {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [savedState, setSavedState] = useState<Partial<PID5TestState> | null>(null);
  ...
  useEffect(() => {
    const checkPreviousCompletion = async () => {
      if (session?.user?.id) {
        try {
          const response = await fetch('/api/reports?testType=pid5');
          if (response.ok) {
            const reports = await response.json();
            // Se ci sono report PID-5, significa che il test è stato completato
            if (reports.length > 0) {
              setHasCompletedTest(true);
              // Rimuovi automaticamente lo stato salvato se il test è già stato completato
              localStorage.removeItem('pid5_test_state');
              setSavedState(null);
            }
          }
        } catch (error) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('pid5_test_state');
    if (saved) {
      try {
  ...
  const handleSave = (state: PID5TestState) => {
    try {
      localStorage.setItem('pid5_test_state', JSON.stringify(state));
    } catch (e) {
  ...
  const handleComplete = async (results: any) => {
    try {
      // Il componente PID5Test ha già salvato i risultati nel database
      ...
      <div className="bg-primary text-white py-4 mb-4">
        <div className="container">
          <div className="d-flex justify-content-between align-items-center">
            <div>
              <h1 className="h2 mb-2">Test PID-5 (Personality Inventory for DSM-5)</h1>
              <p className="mb-0 opacity-75">
                Valutazione completa della personalità secondo il DSM-5
              </p>
            </div>
            <div className="text-end">
              <div className="small opacity-75">Durata stimata</div>
              <div className="h5 mb-0">25-40 min</div>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="alert alert-info mb-4">
            <div className="d-flex align-items-center">
              ...
              <div>
                ...
                <p className="mb-1">
                  Hai un test PID-5 in corso. Puoi continuare da dove avevi lasciato o riiniziare.
                </p>
                <div className="mt-2">
                  <button 
